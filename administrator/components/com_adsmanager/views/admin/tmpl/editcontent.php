...
$query->from($db->quoteName('seeker_adsmanager_ads'));
$query->where($db->quoteName('id') . ' ='.''.$this->content->id.'');
...
// Select all records from the user profile table where key begins with "custom.".
// Order it by the ordering field.
$query->select($db->quoteName(array('id','name','description','price','images')));
$query->from($db->quoteName('seeker_adsmanager_adProduct'));
$query->where($db->quoteName('idAd') . ' ='.''.$this->content->id.'');
