					...
					if (isset($content->images)) {
						...
						foreach($content->images as $img) {
							...
							if (isset($img->tmp) && ($img->tmp == 1)) {
								...
								$thumb = JURI::root()."images/com_adsmanager/ads/".$img->image;
							}
							$index = $img->index;
							echo "<li class='ui-state-default' id='li_img_$index'><img src='".$thumb."?time=".time()."' align='top' border='0' alt='image".$ad_id."' />";
							echo "<br/><input style='margin-top: 1rem;' type='checkbox' name='cb_image$i' onClick='removeImage($index)' value='1' />".JText::_('ADSMANAGER_CONTENT_DELETE_IMAGE').'</li>';
							$currentnbimages++;
							$i++;
						}
					}
...
<?php }
	...
	function displayImageUploaderFormChecker($nbcats=1) {
		?>
